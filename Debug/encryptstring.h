...
// global val taaki we can keep count of rounds and increase to get random many times
__declspec(selectany) int rounds_counter = 10;

namespace crypt {

    // time based seed vs static manual seed
#if 1
...
{
    unsigned int random_number = crypt::linear_congruent_generator(rounds_counter); rounds_counter++; rounds_counter = (rounds_counter % 20) + 10; random_number = random_number % int(limit); return random_number + 1;
};